...
export default async function SubmissionsPage() {
  ...
  const [submissionsRes, calculationsRes, frameworkRes, scoreRes] = await Promise.all([
    supabase.from('submissions').select('*').eq('org_id', userData.org_id).order('created_at', { ascending: false }),
    supabase.from('emission_calculations').select('id, product_id, period, total_co2e, total_embedded, method, is_approved, products(name, cbam_sector, cn_code)').eq('org_id', userData.org_id).eq('period', currentPeriod),
    supabase.from('compliance_frameworks').select('id, name').eq('slug', 'cbam').single(),
    supabase.from('compliance_scores').select('overall_score, risk_level').eq('org_id', userData.org_id).eq('period', currentPeriod).order('version', { ascending: false }).limit(1).maybeSingle(),
  ]);
